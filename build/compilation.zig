...
pub fn addHeader(lib: *std.Build.Step.Compile, source: []const u8, dest: []const u8, ext: []const []const u8) void {
...
pub fn addInclude(lib: *std.Build.Step.Compile, path: []const u8) void {
    ...
    std.debug.print("[{s} include] {s}\n", .{
        lib.name,
        lazy.getPath(builder),
    });
    lib.addIncludePath(lazy);
}

pub fn addSource(lib: *std.Build.Step.Compile, root_path: []const u8, base_path: []const u8, flags: []const []const u8) !void {
    const builder = lib.step.owner;
