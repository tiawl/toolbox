...
pub fn reference(builder: *std.Build, repo: []const u8) ![]const u8 {
    ...
    return std.mem.trim(u8, try builder.build_root.handle.readFileAlloc(builder.allocator, path, std.math.maxInt(usize)), " \n");
}
...
pub const Repository = struct {
    ...
    fn init(builder: *std.Build, name: []const u8, url: []const u8, latest: ?[]const u8, ref: Reference) @This() {
        var self = @This(){
    ...
    // immutable setters
    fn setLatest(self: @This(), builder: *std.Build, latest: []const u8) @This() {
        return init(builder, self.getName(), self.getUrl(), latest, self.getRef());
    }
    ...
    fn searchLatestCommit(self: @This(), builder: *std.Build, tmp: []const u8) !@This() {
    ...
    const Github = struct {
        fn url(builder: *std.Build, name: []const u8) ![]const u8 {
            return try std.fmt.allocPrint(builder.allocator, "https://github.com/{s}", .{
                name,
            });
        }
    };
    ...
    const Gitlab = struct {
        fn url(builder: *std.Build, domain: []const u8, name: []const u8) ![]const u8 {
            return try std.fmt.allocPrint(builder.allocator, "https://gitlab.{s}/{s}", .{
                domain,
                name,
            });
        }
...
pub const Dependencies = struct {
    ...
    // mandatory getters function
    pub fn getIntern(self: @This(), key: []const u8) Repository {
        return self.__intern.get(key).?;
    }
    pub fn getExtern(self: @This(), key: []const u8) Repository {
        return self.__extern.get(key).?;
    }
    pub fn getInterns(self: @This()) std.StringHashMap(Repository).KeyIterator {
        return self.__intern.keyIterator();
    }
    pub fn getExterns(self: @This()) std.StringHashMap(Repository).KeyIterator {
    ...
    pub fn init(builder: *std.Build, pkg_name: []const u8, paths: []const []const u8, intern_proto: anytype, extern_proto: anytype) !@This() {
        ...
        const fetch = builder.option(bool, "fetch", "Update .references folder and build.zig.zon then stop execution") orelse false;
        ...
        inline for (.{
            ...
            inline for (@typeInfo(@TypeOf(proto)).@"struct".fields) |field| {
                ...
                const fork = std.mem.trim(u8, builder.option([]const u8, module_name, "Switch to the given branch from a given fork for the " ++ proto_name ++ " repository") orelse "", " \n");
    ...
    pub fn clone(self: @This(), builder: *std.Build, repo: []const u8, path: []const u8) !void {
        switch (self.getExtern(repo).getRef()) {
            .tag => try run(builder, .{
                .argv = &[_][]const u8{
                    "git",                         "clone",
                    "--branch",                    try reference(builder, repo),
                    "--depth",                     "1",
    ...
    fn fetchIntern(self: @This(), builder: *std.Build, name: []const u8, additional_paths: []const []const u8) !void {
        ...
        try builder.build_root.handle.writeFile(.{
