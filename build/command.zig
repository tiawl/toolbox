...
pub fn run(builder: *std.Build, proc: struct {
    argv: []const []const u8,
    cwd: ?[]const u8 = null,
...
pub fn clean(builder: *std.Build, paths: []const []const u8, extensions: []const []const u8) !void {
    ...
    for (paths) |path| {
        dir = try builder.build_root.handle.openDir(path, .{
            .iterate = true,
        });
        defer dir.close();

        root_path = try builder.build_root.join(builder.allocator, &.{
            path,
        });
